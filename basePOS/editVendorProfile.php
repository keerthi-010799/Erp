...
{ 
   ...
   // }
    ...
    $updatvendorProfile = "UPDATE `vendorprofile` SET `title` = '".$title."', 
						`supname`  = '".$supname."',`suptype`  = '".$suptype."',`blocation`  = '".$blocation."',`status`  = '".$status."',
						`address`  = '".$address."',`city`  = '".$city."',`state`  = '".$state."',`country`  = '".$country."',
						`zip`  = '".$zip."',`workphone`  = '".$workphone."',`mobile`  = '".$mobile."',`web`  = '".$web."',`gstin`  = '".$gstin."',
						`openbalance` = '".$openbalance."',`obasofdate` = '".$obasofdate."',`notes` = '".$notes."',`handler` = '".$handler."'
						 WHERE `id` =". $vendorId;

  echo $updatvendorProfile;
  
  if(mysqli_query($dbcon,$updatvendorProfile))
    {
			...
			<div class="container-fluid">
					...
					<div class="col-xl-12">
							<div class="breadcrumb-holder">
                                    <h1 class="main-title float-left">Edit Vendor </h1>
					...
					<div class="col-xs-12 col-sm-12 col-md-8 col-lg-8 col-xl-8">						
						<div class="card mb-3">
							<div class="card-header">
								...
								<i class="fa fa-pencil-square-o bigfonts" aria-hidden="true"></i>Edit Vendor Profile<span class="text-muted"></span></div></h5>						
							</div>
								
							<div class="card-body">
								<form autocomplete="off" action="editVendorProfile.php" enctype="multipart/form-data" method="post">
								<?php
                                        include("database/db_conection.php");//make connection here
                                        ...
                                        if(isset($_GET['id']))
                                        {
                                            ...
                                            $result = mysqli_query($dbcon, "SELECT vendorid,title,supname,suptype,blocation,
																	status,address,city,state,country,zip,workphone,mobile,email,web,
																	gstin,openbalance,obasofdate
											FROM vendorprofile WHERE id=$id");
                                            while($res = mysqli_fetch_array($result))
                                            {
                                                $supcode = $res['vendorid'];
												$title =	$res['title'];
												$supname 	 =	$res['supname'];	
												//$portal 	 =	$res['portal'];
												$suptype  =	$res['suptype'];	
									...
									<div class="form-group col-md-11">
									  <label for="inputZip">Supplier code</label>
									  <input type="text" class="form-control form-control-sm"  name="supcode"  readonly  value="<?php echo $supcode;?>" />
									</div>
									</div>
									...
									<div class="form-group col-md-8">
									  <label >Supplier Name</span></label>
									  <input type="text" class="form-control form-control-sm" name="supname" value="<?php echo $supname;?>" />
									</div>
									</div>
									  ...
									  <input type="text" class="form-control form-control-sm" name="portal" value="<?php echo $portal;?>" />
										...
										<div class="form-row">
                                            <div class="form-group col-md-11">
                                                ...
                                                <select id="locname" onchange="onlocode(this)" class="form-control form-control-sm" name="suptype">
                                                    <?php 
                                                    include("database/db_conection.php");//make connection here

                                                    $sql = mysqli_query($dbcon, "SELECT suptype FROM suptype");
                                                    while ($row = $sql->fetch_assoc()){	
                                                        echo $suptype_get=$row['suptype'];
                                                         if($suptype_get==$suptype){
                                                            echo '<option value="'.$suptype_get.'" selected>'.$suptype_get.'</option>';

                                                        }else{
                                                            echo '<option value="'.$suptype_get.'" >'.$suptype_get.'</option>';
								...
								<div class="checkbox"><label>
								 <input type="checkbox" name="primaryflag" value="0" class="ember-checkbox ember-view">Make this profile as primary for all correspondence</label>
                                    ...
                                    <div class="form-group text-right m-b-10">
                                        <input type="hidden" name="vendorId" value="<?=$_GET['id']?>">
                                        &nbsp;<button class="btn btn-primary" name="vendorProfEdit" type="submit">
                                        Update
                                        </button>                                                       
                                    </div>
                                </div>
