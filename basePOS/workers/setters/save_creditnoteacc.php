    ...
    if($action=="add"){
        ...
        if (mysqli_query($dbcon,$sql2)) {
            ...
            if($return['status']===true){
                ...
                $sql0 = " UPDATE invoicesacc SET inv_balance_amt =  inv_balance_amt - ".$obj['creditnote_value']."  WHERE inv_code='".$obj['creditnote_inv_code']."' ;";

                mysqli_query($dbcon,$sql0);    

                update_open_balance($dbcon,'recievables');

                $inv_val_arr = findbyand($dbcon,$obj['creditnote_inv_code'],"invoicesacc","inv_code");

                if($inv_val_arr['values'][0]['inv_balance_amt']==0){
                    $return = updatebyand($dbcon,"Paid","invoicesacc","inv_payment_status","inv_code",$obj['creditnote_inv_code']);
                    ...
                    $return = updatebyand($dbcon,"Partially Paid","invoicesacc","inv_payment_status","inv_code",$obj['creditnote_inv_code']);
                }else if($inv_val_arr['values'][0]['inv_balance_amt']==$inv_val_arr['values'][0]['inv_value']){
                    $return = updatebyand($dbcon,"UnPaid","invoicesacc","inv_payment_status","inv_code",$obj['creditnote_inv_code']);
                }
